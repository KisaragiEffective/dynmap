            ...
            if (pendingcoords.contains(coord)) {
                for (Object o : chunkstoremove.values()) {
                    if (chunkCoord == null) {
                        ...
                        Arrays.stream(fields).forEachOrdered(f -> {
                            if ((chunkCoord == null) && (f.getType().equals(ChunkPos.class))) {
                                chunkCoord = f;
                                chunkCoord.setAccessible(true);
                            } else if ((nbtTag == null) && (f.getType().equals(NBTTagCompound.class))) {
                                nbtTag = f;
